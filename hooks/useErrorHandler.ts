...
export function useErrorHandler() {
  const handleError = useCallback(async (
    ...
    try {
      // Log to database
      await logToDatabase(level, `Client Error: ${context}`, errorReport)
    } catch (logError) {
      if (!silent) {
...
export function useComponentErrorHandler(componentName: string) {
  const { handleError, handleAsyncError, safeExecute } = useErrorHandler()
  ...
  const handleComponentError = useCallback((error: Error, additionalContext?: string) => {
    ...
    return handleError(error, { context, level: LogLevel.ERROR })
  ...
  const handleComponentAsyncError = useCallback((
    ...
    return handleAsyncError(asyncFn, { context, level: LogLevel.ERROR, retryable: true })
  ...
  const safeComponentExecute = useCallback(<T>(
    ...
    return safeExecute(fn, fallback, { context, level: LogLevel.ERROR })
