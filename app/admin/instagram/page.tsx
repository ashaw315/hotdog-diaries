...
import { InstagramScanConfig, InstagramScanStats, InstagramScanResult } from '@/lib/services/instagram-scanning'
...
export default function InstagramSettingsPage() {
  ...
  const [config, setConfig] = useState<InstagramScanConfig | null>(null)
  const [stats, setStats] = useState<InstagramScanStats | null>(null)
  const [status, setStatus] = useState<InstagramApiStatus | null>(null)
  const [scanHistory, setScanHistory] = useState<InstagramScanResult[]>([])
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: 'Failed to load Instagram data' })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const loadConfig = async () => {
    try {
      ...
      const data: ApiResponse<InstagramScanConfig> = await response.json()
  ...
  const loadStats = async () => {
    try {
      ...
      const data: ApiResponse<InstagramScanStats> = await response.json()
      if (data.success && data.data) {
        setStats(data.data)
      }
    } catch (error) {
  ...
  const loadStatus = async () => {
    try {
      ...
      const data: ApiResponse<InstagramApiStatus> = await response.json()
      if (data.success && data.data) {
        setStatus(data.data)
      }
    } catch (error) {
  ...
  const loadScanHistory = async () => {
    try {
      const response = await fetch('/api/admin/instagram/scan-history')
      const data: ApiResponse<InstagramScanResult[]> = await response.json()
      if (data.success && data.data) {
  ...
  const testConnection = async () => {
    ...
    try {
      const response = await fetch('/api/admin/instagram/test-connection')
      const data: ApiResponse = await response.json()
  ...
  const runManualScan = async () => {
    ...
    try {
      const response = await fetch('/api/admin/instagram/scan', {
        method: 'POST'
      })
  ...
  const startAuthentication = async () => {
    ...
    try {
      const response = await fetch('/api/admin/instagram/authenticate', {
        method: 'POST'
      })
  ...
  const updateConfig = (field: keyof InstagramScanConfig, value: any) => {
    if (!config) return
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-gray-600">Loading Instagram settings...</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Instagram Integration</h1>
        <p className="mt-2 text-gray-600">
          Configure Instagram content scanning for hotdog discovery
        </p>
      </div>
            ...
            <div>
              ...
              <div className="mt-2 flex items-center">
                <div className={`w-3 h-3 rounded-full mr-2 ${
                  status?.isAuthenticated ? 'bg-green-400' : 'bg-red-400'
                }`}></div>
                <span className={`text-sm ${
                  status?.isAuthenticated ? 'text-green-600' : 'text-red-600'
                }`}>
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <input
                      ...
                      onChange={(e) => updateConfig('includeStories', e.target.checked)}
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                    />
                  </div>
                </div>
        ...
        <div className="bg-white shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            ...
            <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                <dt className="text-sm font-medium text-gray-500">Average Likes</dt>
                <dd className="mt-1 text-3xl font-semibold text-gray-900">{stats.averageLikes.toFixed(1)}</dd>
              </div>
