...
export async function POST(request: NextRequest) {
  // Emergency bypass authentication for critical fix
  ...
  try {
    ...
    const youtubeVideos = [
      ...
      },
      {
        ...
        video_url: 'https://www.youtube.com/watch?v=HBNE',
      ...
      try {
        const result = await db.query(`
          INSERT INTO content_queue (
            ...
            is_approved, confidence_score, scraped_at, created_at, updated_at
    ...
    for (const gif of giphyGifs) {
      try {
        ...
        console.error(`❌ Failed to add GIF: ${gif.title}`, error)
    ...
    return NextResponse.json({
      success: true,
      message: 'Emergency content balance fix completed successfully',
      results: {
        videosAdded: results.videosAdded,
        gifsAdded: results.gifsAdded,
        totalAdded: results.totalAdded,
        errors: results.errors
      },
      newBalance: newBalance.rows,
