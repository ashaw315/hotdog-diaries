...
interface TokenConfig {
  ...
  description: string
  exampleValue?: string
  format: 'hex' | 'base64' | 'alphanumeric' | 'jwt'
...
const TOKEN_CONFIGS: Record<string, TokenConfig> = {
  ...
  AUTH_TOKEN: {
    name: 'AUTH_TOKEN', 
    length: 100,
    description: 'Admin API authentication token (JWT format)',
    format: 'jwt',
...
class TokenRotator {
   ...
   */
  generateToken(length: number, format: 'hex' | 'base64' | 'alphanumeric' | 'jwt' = 'hex'): string {
    ...
    switch (format) {
      ...
      case 'jwt':
        // For JWT format, generate a hex token that can be used as a JWT secret
        // The actual JWT will be generated by the auth service
        return bytes.toString('hex').substring(0, length)
