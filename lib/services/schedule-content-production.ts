 ...
 * Fixed to work with Supabase production schema
 */
...
/**
 * Get recently posted content to enforce platform diversity (PRODUCTION SCHEMA)
 */
...
/**
 * Schedule content for a specific day (PRODUCTION SCHEMA)
 */
async function scheduleDayContent(
  ...
  try {
    ...
    for (let i = 0; i < Math.min(selectedContent.length, POSTS_PER_DAY); i++) {
      ...
      // Update database with schedule using production field names
      await db.query(`
        UPDATE content_queue 
        SET scheduled_post_time = $1, content_status = $2, updated_at = NOW()
        WHERE id = $3
      `, [
